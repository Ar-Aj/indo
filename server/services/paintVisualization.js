...
class PaintVisualizationService {
  ...
  async generatePlainPaintedWall(originalImagePath, maskImagePath, colorHex, colorName) {
    ...
    const plainPayload = {
      ...
      prompt: `paint this wall uniform ${colorName} color ${colorHex}, exact hex color ${colorHex}, flat consistent color across entire wall, ignore room lighting, solid paint coverage`,
      negative_prompt: 'keep original color, no color change, same wall color, furniture, objects, people, text, architectural changes, lighting effects, shadows, bright spots, color variation, color shift',
      strength: 0.75,
      guidance: 20.0,
      steps: 40,
      width: newWidth,
