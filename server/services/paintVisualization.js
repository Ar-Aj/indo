...
class PaintVisualizationService {
  ...
  async generatePlainPaintedWall(originalImagePath, maskImagePath, colorHex, colorName) {
    ...
    const plainPayload = {
      ...
      prompt: `wall painted in ${colorName} paint color ${colorHex}, professional wall paint finish, realistic indoor lighting, maintain room structure`,
      negative_prompt: 'windows, lamps, furniture, decorations, objects, paintings, frames, new items, extra objects, people, text, cartoon, color shift, wrong color, different color, lighting fixtures, architectural changes',
      strength: 0.95,
      guidance: 12.0,
      steps: 30,
      width: newWidth,
